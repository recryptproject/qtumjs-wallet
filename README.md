...
This is a client-side wallet library that can generate private keys from a mnemonic, or import private keys from other RECRYPT wallets.

It can sign transactions locally, and submit the raw transaction data to a remote recrypt node. The blockchain data is provided by the Insight API (which powers https://explorer.recrypt.org/), rather than the raw recryptd RPC calls.

This library makes it possible to run DApp without the users having to run a full recryptd node.

> This library is extracted from the official [RECRYPT web wallet](https://github.com/recryptproject/recrypt-web-wallet).

## Install

```
yarn add recryptjs-wallet
```
...
* Uses satoshi (1e8) as internal units
  * Can represent up to ~90 million RECRYPT accurately.
...
```js
import { networks, generateMnemonic } from "recryptjs-wallet"
...
The currency unit used is `satoshi`. To convert recrypt to satoshi you should multiply the amount you want with `1e8`.

```js
import { networks } from "recryptjs-wallet"
...
The ABI encoding for the `burnbabyburn()` invokation is `e179b912`. We'll burn 0.05 recrypt, expressed in unit of satoshi.
...
async function main() {
  ...
  const tx = await wallet.contractSend(contractAddress, encodedData, {
    amount: 0.05 * 1e8, // 0.05 recrypt in satoshi
  })
...
Suppose you want to import the public address `qg3HYD8c4bAVLeEzA9t3Ken3Y3Mni1HZSS`. Use `recrypt-cli` to dump the private key from wallet:
...
const amount = 0.15 * 1e8 // 0.15 RECRYPT
...
```ts
const tx = await wallet.send(toAddress, amount, {
  // rate is 400 satoshi per byte, or  ~0.004 recrypt/KB, as is typical.
  feeRate: 400,
})
```
...
You can decode the raw transaction using `recrypt-cli`:

```
recrypt-cli decoderawtransaction 0100000001a09a921ecb38...
...
```
recrypt-cli decoderawtransaction 0100000001e7b6f2767e8a6...
```
